...
function extractNameFromTemplate(value) {
    return value.replace(/Hello,/g, '').replace(/!/g, '').replace(/^ /g, '');
}
...
function removeFirstOccurrences(str, value) {
    return str.replace(value, '').replace(/ {2,}/, ' ');
}
...
function encodeToRot13(str) {
    var result = str.replace(/[a-zA-Z]/g,function(c){return String.fromCharCode((c<="Z"?90:122)>=(c=c.charCodeAt(0)+13)?c:c-26);});
    return result;

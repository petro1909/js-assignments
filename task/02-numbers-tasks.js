...
function getAngleBetweenVectors(x1, y1, x2, y2) {
    ...
    var moda = Math.sqrt(Math.pow(x1,2)+Math.pow(y1,2));
    var modb = Math.sqrt(Math.pow(x2,2)+Math.pow(y2,2));
    var cos = scalmult/(moda*modb);
...
function roundToPowerOfTen(num, pow) {
    var res1 = Math.round(num/Math.pow(10,pow))
    return res1*Math.pow(10,pow);
}
